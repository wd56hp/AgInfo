...
.\backup.ps1 -BackupDir "C:\Backups\AgInfo"

# Skip GeoServer or Web files (faster backups)
.\backup.ps1 -SkipGeoServer
.\backup.ps1 -SkipWeb
```
...
# Override backup directory (takes precedence over .env)
./backup.sh --backup-dir /backups/aginfo

# Skip GeoServer or Web files
./backup.sh --skip-geoserver
./backup.sh --skip-web
```
...
```powershell
# Restore from backup
.\restore.ps1 -BackupPath "backups\aginfo_backup_20240101_120000"

# Restore with force (skip confirmation)
.\restore.ps1 -BackupPath "backups\aginfo_backup_20240101_120000" -Force

# Restore specific components only
.\restore.ps1 -BackupPath "backups\aginfo_backup_20240101_120000" -SkipGeoServer
.\restore.ps1 -BackupPath "backups\aginfo_backup_20240101_120000" -SkipWeb
.\restore.ps1 -BackupPath "backups\aginfo_backup_20240101_120000" -SkipDatabase
```
...
# Restore from backup
./restore.sh --backup-path "backups/aginfo_backup_20240101_120000"

# Restore with force (skip confirmation)
./restore.sh --backup-path "backups/aginfo_backup_20240101_120000" --force

# Restore specific components only
./restore.sh --backup-path "backups/aginfo_backup_20240101_120000" --skip-geoserver
./restore.sh --backup-path "backups/aginfo_backup_20240101_120000" --skip-web
./restore.sh --backup-path "backups/aginfo_backup_20240101_120000" --skip-database
```
...
6. Arguments: `-File "C:\path\to\AgInfo\backup.ps1" -Compress`
...
```bash
0 2 * * * /path/to/AgInfo/backup.sh --compress >> /var/log/aginfo-backup.log 2>&1
```
